...
(function() {
  ...
  Array.prototype.group_by = function(f) {
    return this.reduce(function(r, x) {
      var k;
      k = f(x);
      r[k] = r[k] || [];
      r[k].push(x);
  ...
  render_prior_thingset = function(thingset, parent) {
    var details, list, summary, thing, _i, _len, _ref;
  ...
  render_group_of_prior_thingsets = function(month, thingsets) {
    var details, prior, summary, thingset, _i, _len;
    prior = d.getElementById('prior');
    details = d.createElement('details');
    summary = d.createElement('summary');
    summary.appendChild(d.createTextNode(month));
    details.appendChild(summary);
    for (_i = 0, _len = thingsets.length; _i < _len; _i++) {
      thingset = thingsets[_i];
      render_prior_thingset(thingset, details);
    }
    prior.appendChild(details);
  };

  render_prior_things = function(prior_thingsets) {
    var grouped_thingsets, month, months, sorted_thingsets, things;
    sorted_thingsets = prior_thingsets.sort(function(a, b) {
      if (a.date < b.date) {
        return 1;
    ...
    grouped_thingsets = sorted_thingsets.group_by(function(thingset) {
      var date;
      date = new Date(thingset.date);
      return months[date.getMonth()] + ' ' + date.getFullYear();
    });
    for (month in grouped_thingsets) {
      things = grouped_thingsets[month];
      render_group_of_prior_thingsets(month, things);
    }
  };
